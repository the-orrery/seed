...
> **退役中 (ADR-050 VP3, REFIT-21 批3)**。平台正向单仓 monorepo (`orrery`) 收编, seed 的存在前提 (polyrepo 出生地 + 跨仓漂移审计) 随之蒸发, copier 语义复刻是负资产;脚手架职能改由 workspace 约定承载, 新建仓数=0 → `seed new` 零前向需求。退役是进行中的流程 (非完成态): 见下方值守/归档口径。
>
